...
namespace ComicVine.API.Testing.Repositories.Powers
{
    ...
    public static class PowersMockingSetup
    {
        public static Mock<DataModel.ModelEntities> DoMockingSetupForContext(bool includePowersData, out Mock<IDbSet<Power>> mockSetPowers, bool markAPowerInactive = false)
        {
            ...
            {
                new Power { Id = 1, Active = !markAPowerInactive, CreatedDate = createDate, CustomKey = "KING-STEPHEN", ApiDetailUrl = "/TEST/KING-STEPHEN", Name = "Stephen King", Description = "a famous novelist" },
                new Power { Id = 2, Active = true, CreatedDate = createDate.AddDays(1), CustomKey = "NIVEN-LARRY", ApiDetailUrl = "/TEST/NIVEN-LARRY", Name = "Larry Niven", Description = "a novelist" }
            }).AsQueryable();
    ...
    public class PowersRepositoryTests
    {
        ...
        public void Verify_Add_Should_AddTheEntityToTheContext()
        {
            ...
            var powers = new Power { Active = true, CustomKey = "SALVATORE-RAA", Name = "R.A.A Salvatore", Description = "a novelist" };
            // Act
        ...
        public void Verify_Get_ByID_Should_ReturnTheCorrectPower()
        {
            ...
            var powers = repository.Get(1);
            // Assert
            Assert.Equal("Stephen King", powers.Name);            Assert.Equal("/TEST/KING-STEPHEN", powers.ApiDetailUrl);
        }
        ...
        public void Verify_List_Should_ReturnAListOfPowersWithData()
        {
            ...
            Assert.Equal("Stephen King", powers[0].Name);            Assert.Equal("/TEST/KING-STEPHEN", powers[0].ApiDetailUrl);
            Assert.Equal(2, powers[1].Id);
            Assert.Equal("Larry Niven", powers[1].Name);            Assert.Equal("/TEST/NIVEN-LARRY", powers[1].ApiDetailUrl);
        }
        ...
        public void Verify_Search_Should_ReturnAListOfPowersWithDataMatchingSearchParameters()
        {
            ...
            Assert.Equal("Larry Niven", powers[0].Name);            Assert.Equal("/TEST/NIVEN-LARRY", powers[0].ApiDetailUrl);
        ...
        public void Verify_Search_WithSelectStatement_Should_ReturnAListOfDynamicObjects()
        {
            ...
            var searchModel = new Mock<IPowerSearchModel>();
            // Act
            var powers = repository.Search(searchModel.Object, i => new { i.Id, MyApiDetailUrl = i.ApiDetailUrl, MyName = i.Name }).ToArray();
            // Assert
            Assert.Equal(1, powers.Length);
            Assert.Equal(2, powers[0].Id);
            Assert.Equal("Larry Niven", powers[0].MyName);            Assert.Equal("/TEST/NIVEN-LARRY", powers[0].MyApiDetailUrl);
            // Stephen King was filtered out because he was Inactive
        }
